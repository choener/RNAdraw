...
sHairpin :: Candidates
sHairpin = pure $ (xs, hardBox xs) where xs = [(0,0),(0,1)]

-- |

sStem :: Candidates -> Candidates
sStem cs = map f cs where
...
sUnpLoop :: Int -> Int -> Candidates -> Candidates
sUnpLoop i j cs
  | i==0 && j==0 = sStem cs
  | otherwise = sStem cs -- TODO

-- |

sMultibranched :: [Region] -> Candidates
sMultibranched rs = map go $ omega ps where
